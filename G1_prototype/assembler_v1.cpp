...
void load_address(int start){
    ...
    if(line.length() > start-1){
        ...
        if(line[start] == '$'){
            ...
            if(line.length()-start < 5){
                error.push_back(6);
                error_strings.push_back("");
                error_line.push_back(line_number);
            }
        }
        else{
    ...
    while(getline(file, line)){
        ...
        if(line == "" or line[0] == '#'){
            ...
            for(int i = 1; i < line.size()-1; i++){
                ...
                if(line[i] == ' '){
                    ...
                    if(isdigit(line[i+1])){
                        ...
                        << std::hex << address - 1 + stoi(line.substr(i+1, line.size()-1));
            ...
            if(skip == false){
                ...
                stringstream stream;
                stream << std::hex << std::setfill ('0') << std::setw(sizeof(address))  << std::hex << address - 1;
                label_address.push_back(stream.str());
            ...
            if(line[4] == 'A'){
                if(line[6] == 'T'){
                    ...
                    error.push_back(4);
                    error_strings.push_back(line.substr(4, 1));
                    error_line.push_back(line_number);
    ...
    for(int i = 0; i < missing_label.size(); i++){
        ...
        for(int j = 0; j < labels.size(); j++){
            ...
            if(missing_label[i] == labels[j]){
                ...
                hexcode.replace(missing_label_position[i], 4, label_address[j]);
...
if(error.size() > 0){
    ...
    for(int i = 0; i < error.size(); i++){
        ...
        switch (error[i]) {
            ...
            case 0:
                cout<<"at line "<<error_line[i]<<", instruction \""<<error_strings[i]<<"\" does not exist.";
                break;

            case 1:
                cout<<"at line "<<error_line[i]<<", multiple instances of label \""<<error_strings[i]<<"\".";
                break;

            case 2:
                cout<<"at line "<<error_line[i]<<", label \""<<error_strings[i]<<"\" does not exist.";
                break;
            ...
            case 4:
                cout<<"at line "<<error_line[i]<<", register \""<<error_strings[i]<<"\" is not valid.";
                break;

            case 5:
                cout<<"at line "<<error_line[i]<<", missing label or address for instruction.";
                break;

            case 6:
                cout<<"at line "<<error_line[i]<<", incomplete address.";
                break;
        }
    ...
    cout<<"There were 0 errors found.";
